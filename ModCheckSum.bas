...
'Pengecekan virus dengan algoritma 3EB (3 Executable Binaries)
If IsExe(lpFileName) = 0 Then Exit Function
    SaveStringNow(1) = CalcBinary(lpFileName, 199, 4000)
If InStrRev(IntViriString1, SaveStringNow(1)) = 0 Then GoTo VariantCheck
    SaveStringNow(2) = CalcBinary(lpFileName, 199, 4250)
If InStrRev(IntViriString2, SaveStringNow(2)) = 0 Then GoTo VariantCheck
    SaveStringNow(3) = CalcBinary(lpFileName, 199, 4500)
If InStrRev(IntViriString3, SaveStringNow(3)) = 0 Then GoTo VariantCheck

SaveStringNow(4) = SaveStringNow(1) & SaveStringNow(2) & SaveStringNow(3)
For i = 0 To UBound(ViriString4)
    If SaveStringNow(4) = ViriString4(i) Then
        AnalyzeFile = ViriName(i)
        Exit Function
    End If
Next i

'Pengecekan varian virus dengan algoritma EIB (Executable Icon Binary)
VariantCheck:
    ...
    If InStrRev(IntViriIconID, IconIDNow) <> 0 Then
        For i = 0 To UBound(ViriIconID)
            If ViriIconID(i) = IconIDNow Then
                AnalyzeFile = ViriVariantName(i) & ".Variant"
...
FormScan.sIcon.BackColor = vbWhite
DrawIconEx FormScan.sIcon.hDC, 0, 0, hIcon, 0, 0, 0, 0, DI_NORMAL
DestroyIcon hIcon

PicPath = GetSpecPath(SmadTempDir) & "\" & GetFileName(lpFileName) & ".tmp"
SavePicture FormScan.sIcon.Image, PicPath
